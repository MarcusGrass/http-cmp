...
use http_test_util::{
    byte_body, empty_body, GetCounterResponse, IncrementCounterRequest, IncrementCounterResponse,
...
pub(crate) async fn run(
    ...
    tests_per_task: usize,
    client: HttpClient,
    base_uri: &'static str,
) -> anyhow::Result<()> {
    let mut tasks = Vec::with_capacity(num_concurrent);
    for _ in 0..num_concurrent {
        tasks.push(tokio::spawn(run_sequential(
            tests_per_task,
            client.clone(),
            base_uri,
        )));
    ...
    for res in results.iter() {
        update_stats(
            res.get_index_rtt.as_micros(),
            &mut min_get_index_rtt,
            &mut max_get_index_rtt,
            &mut total_get_index,
        );
        update_stats(
            res.get_count_rtt.as_micros(),
            &mut min_get_count_rtt,
            &mut max_get_count_rtt,
            &mut total_get_count,
        );
        update_stats(
            res.put_count_rtt.as_micros(),
            &mut min_put_count_rtt,
            &mut max_put_count_rtt,
            &mut total_put_count,
        );
        update_stats(
            res.post_count_rtt.as_micros(),
            &mut min_post_count_rtt,
            &mut max_post_count_rtt,
            &mut total_post_count,
        );
        update_stats(
            res.total().as_micros(),
            &mut min_total_rtt,
            &mut max_total_count_rtt,
            &mut total_total_count,
        );
    }
...
pub(crate) async fn run_sequential(
    ...
    client: HttpClient,
    base_uri: &'static str,
) -> anyhow::Result<Vec<SingleRunStatistics>> {
...
pub(crate) async fn run_full(
    ...
    let _resp: GetCounterResponse =
        serde_json::from_slice(&resp).context("Failed to serialize get count")?;

    let put_body = serde_json::to_vec(&IncrementCounterRequest::new(15))
        .context("Failed to create put request")?;
    ...
    let post_body = serde_json::to_vec(&SwapCounterRequest::new(155))
